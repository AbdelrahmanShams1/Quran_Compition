          ...
          <div className="space-y-6 mt-6">
            {data.map((section) => (
              <div key={section.key} className="space-y-4">
                <h3 className="text-xl font-bold text-indigo-900">{section.name}:</h3>

                {section.categories ? (
                  section.categories
                    .filter((category) => category.key === userGender)
                    .map((category) => (
                      <div key={category.key} className="bg-blue-50 p-4 rounded-lg space-y-2">
                        <h4 className="text-lg font-bold text-indigo-700">{category.name}</h4>
                        {category.types.map((type) => (
                          ...
                          >
                            ...
                            <span className="font-bold">{type.points} نقطة:</span> {type.name}
                          </p>
                        ))}
                      </div>
                    ))
                  ...
                  <div className="bg-blue-50 p-4 rounded-lg space-y-2">
                    {section.details.map((item) => (
                      ...
                      >
                        ...
                        {item.points ? (
                          <>
                            <span className="font-bold">{item.points} نقطة:</span> {item.name}
                          </>
                        ) : (
                          item.name
                        )}
